...
//            e.preventDefault();
            ...
            var parent_lis = the.parents('li.nav-item');
            var page_bar_html = [];
            parent_lis.each(function(i) {
            	if (0 == i) {
            		page_bar_html.push('<li><span>' + $(this).find('>a').text() + '</span></li>');
            	} else {
            		page_bar_html.push('<li><a>' + $(this).find('>a').text() + '</a><i class="fa fa-circle"></i></li>');
            	}
            });
...
//            e.preventDefault();
        ...
        $('.page-content').on('click', '.skip_to_edit', function(event) {
        	event.preventDefault();
        	var url = $(this).attr('href');
        	if (!url) {
        		url = $(this).attr('url');
        	}
        	$('.page-content > .page-edit-body').load(url, function() {
        		$('.page-content > .page-content-body').slideUp();
        		$('.page-content > .page-edit-body').slideDown();
        		if (currentModule.initEdit) {
        			currentModule.initEdit();
        		}
        	});
    	}).on('click', '.skip_to_content', function(event) {
    		event.preventDefault();
    		$('.page-content > .page-content-body').slideDown();
    		if (currentModule.destroyEdit) {
    			currentModule.destroyEdit();
    		}
    		$('.page-content > .page-edit-body').slideUp().empty();
    	});
